...
class DailyConlusion:
    ...
    def __init__(self, groupId):
        self.__configuration = JsonUtils.json2Dict(os.path.join(
            os.path.join(os.getcwd(), 'cn', 'acmsmu', 'FG', 'data', 'config.json')))
        self.__groupId = groupId
        dataDict = IOUtils.deserializeObjFromPkl(os.path.join(os.path.join(
            os.getcwd(), 'cn', 'acmsmu', 'FG', 'data', self.__groupId, 'var.pkl')))
        # 确定使用哪个文件
                ...
                for eachLine in f:
                    ...
                    if isFirst:
                        ...
                        if re.search('^\d{4}-\d{2}-\d{1,2} \d{1,2}:\d{2}:\d{2} \d{5,11}', eachLine) is None:
                            ...
                            # 过滤URL
                            eachLine = re.sub(
                                '(https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]', '', eachLine)
                            # 特殊情况过滤
    ...
    def __generateWC(self):
        ...
        imageU = os.path.join(os.getcwd(), 'cn', 'acmsmu', 'FG',
                              'data', 'assets', 'images', todayMask['fileNameU'])
        desc = todayMask['desc']
        ...
        try:
            ...
            wc = WordCloud(font_path=fontPath, mask=mask,
                           background_color='white')
            wc.generate_from_frequencies(wordDic)
            figName = time.strftime(
                "%Y-%m-%d%H-%M-%S", time.localtime())+'-'+str(round(random.uniform(0, 100)))+'.png'
            wc.to_file(os.path.join(os.getcwd(), 'cn', 'acmsmu',
                       'FG', 'data', 'assets', 'wc', figName))
            imginfo.append('[CQ:image,file=file:///' + os.path.join(os.getcwd(),
                           'cn', 'acmsmu', 'FG', 'data', 'assets', 'wc', figName) + ']')
            imginfo.append('[CQ:image,file=file:///' + os.path.join(os.getcwd(), 'cn',
                           'acmsmu', 'FG', 'data', 'assets', 'images', todayMask['fileNameO']) + ']')
            imginfo.append(desc)
            for i in range(3):
